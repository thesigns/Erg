...
public class Tile
{
    ...
    public Glyph GetDisplayGlyph()
    {
        ...
        bool isWater = Type == TileType.ShallowWater || Type == TileType.DeepWater;

        if (Items.Count >= 2)
        {
            uint foreground = isWater ? 0x87CEEBFF : 0x8B4513FF;
            return new Glyph('%', foreground, 0x000000FF);
        }

        if (Items.Count == 1)
        {
            if (isWater)
                return new Glyph(Items[0].Glyph.Character, 0x87CEEBFF, 0x000000FF);
    ...
    public static Tile ShallowWater =>
        new('≈', 0x3256FFFF, 0x000000FF, true, true, "Shallow Water", TileType.ShallowWater);
